...
def create_payment(request):
    print(request.data,'data of payment')
    user = request.user  # Or get from request session
    rental_id = request.POST.get('rental_id')  # Assuming rental info is passed
    ...
    payment = Payment.objects.create(
        user=user,
        rental=rental,
        transaction_uuid="some_unique_transaction_uuid",  # Generated by the payment gateway
        amount=rental.price,  # Assuming rental has a price field
        status='pending',  # Initially set to 'pending'
    )

    # Redirect user to the payment gateway with the payment details
    payment_gateway_url = "https://paymentgateway.com/checkout"
